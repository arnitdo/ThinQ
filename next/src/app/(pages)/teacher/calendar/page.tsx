...
export default function Page() {
  ...
  useEffect(() => {
    setShowData(data.filter((item) => {
      return new Date(item.lectureStartTimestamp).getDate() === currDate.getDate()
    }))
  }, [data, currDate])
  ...
  const day = (new Date(currDate)).toLocaleDateString()===(new Date()).toLocaleDateString()?"Today":(new Date(currDate)).toLocaleDateString()===((new Date(Date.now()+(1000*60*60*24)))).toLocaleDateString()?"Tomorrow":(new Date(currDate)).toLocaleDateString()===((new Date(Date.now()-(1000*60*60*24)))).toLocaleDateString()?"Yesterday":days[(new Date(currDate)).getDay()]
  return (
    <>
          ...
          <div className="flex justify-between items-end mb-7 out">
            <div>
              ...
              <div className="flex gap-4">
                <h2 className="text-[1.75rem] font-medium text-[#575757]">{(new Date(currDate)).toDateString().slice(4)}</h2>
                <div className="dayChangeButtons flex">
                  <button onClick={()=>setCurrDate(new Date(currDate.getTime()-(1000*60*60*24)))} className="py-1 px-[0.3125rem] bg-[hsl(0,0%,95%)] hover:bg-[hsl(0,0%,85%)] rounded-[0.5rem_0_0_0.5rem]">
                    <img src="/calendarDayNavigation.svg" alt="" />
                  </button>
                  <button onClick={()=>setCurrDate(new Date(currDate.getTime()+(1000*60*60*24)))} className="py-1 px-[0.3125rem] bg-[hsl(0,0%,95%)] hover:bg-[hsl(0,0%,85%)] rounded-[0_0.5rem_0.5rem_0]">
            ...
            <button onClick={()=>{setShowDatePicker(!showDatePicker)}} className="calendarIcon | bg-[hsl(0,0%,95%)] relative hover:bg-[hsl(0,0%,85%)] p-[0.625rem] rounded-[0.5rem]">
              ...
              <div className="lectureCard | grid hover:gap-[0.9375rem] grid-cols-[0_1fr] hover:grid-cols-[0.5rem_1fr] transition-[grid-template-columns]">
                ...
                <div className="p-[0.9375rem] border border-[#7C7A7A] rounded-[0.3125rem] bg-[hsl(0,0%,95%,20%)] hover:shadow-[0_8px_15.1px_hsl(0,1%,25%,10%)] flex justify-between items-center">
                  <div>
                    ...
                    <h3 className="text-[0.9375rem] text-[#A1A1A1]">Classroom: <span>{item.lectureClassroom.classroomName}</span></h3>
