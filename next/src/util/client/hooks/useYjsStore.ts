...
export function useYjsStore({
	...
	const { yDoc, yStore, meta, room } = useMemo(() => {
		...
		const yStore = new YKeyValue(yArr as any)
	...
	useEffect(() => {
		...
		function handleSync() {
			...
			const handleChange = (
				...
				changes.forEach((change: any, id: any) => {
			...
			const handleMetaUpdate = () => {
				const theirSchema = meta.get('schema') as any
				if (!theirSchema) {
			...
			if (yStore.yarray.length) {
				...
				const records = yStore.yarray.toJSON().map(({ val }: {val: any}) => val)
